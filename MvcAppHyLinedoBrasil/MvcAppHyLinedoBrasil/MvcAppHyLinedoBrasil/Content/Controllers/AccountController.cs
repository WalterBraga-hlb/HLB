...
namespace MvcAppHyLinedoBrasil.Controllers
{
    public class AccountController : Controller
    {
        ...
        public void CarregaLayout()
        {
            ...
            {
                Session["logo"] = "BR";
                ViewBag.Message = "Hy-Line do Brasil - Apps";
            }
            if (Session["empresa"].ToString() == "LB")
            {
                Session["logo"] = "LB";
                ViewBag.Message = "Lohmann do Brasil - Apps";
            }
            if (Session["empresa"].ToString() == "HN")
            {
                Session["logo"] = "HN";
                ViewBag.Message = "H&N Avicultura - Apps";
            }
            if (Session["empresa"].ToString().Count() > 2)
            {
                Session["logo"] = "EW";
                ViewBag.Message = "EW Group Brasil - Apps";
            }
            if (Session["empresa"].ToString().Contains("PL"))
            {
                Session["logo"] = "PL";
                ViewBag.Message = "Planalto Postura - Apps";
            }
        }
    }
}
